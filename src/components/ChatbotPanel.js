...
import { X, Send, Loader2, Dumbbell, Utensils, Zap, Phone } from "lucide-react";
...
export default function ChatbotPanel({ onClose }) {
  ...
  const callAI = async (query) => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { role: "ai", content: "Error connecting to the AI coach." },
      ]);
    } finally {
      setIsLoading(false);
    }
  ...
  const handleSend = async (text) => {
    ...
    // Lead Capture Logic (pause AI until name+phone collected)
    if (!leadData.captured) {
      if (leadData.step === "name") {
        ...
        if (!leadData.originalQuery) {
          ...
          setMessages((prev) => [
            ...prev,
            { role: "ai", content: "Before I create your personalized plan, can I get your name?" },
          ]);
        } else {
          ...
          setMessages((prev) => [
            ...prev,
            { role: "ai", content: `Great ${messageToSend}! Please enter your WhatsApp number.` },
          ]);
        }
        ...
        setMessages((prev) => [
          ...prev,
          { role: "ai", content: "Thanks! Creating your personalized plan now... We'll send it on WhatsApp too! 🚀" },
        ]);
